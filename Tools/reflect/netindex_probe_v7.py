...
def probe_playercontroller_open(
    ...
    # Discover UBoolProperty::BitMask from reflection itself:
    # Core.BoolProperty.BitMask is an IntProperty whose PropertyOffset tells
    # us where the native member lives inside UBoolProperty.
    bitmask_member_prop = _find_object_by_path(
        objs,
        groups,
        "Core.BoolProperty.BitMask",
    )

    bitmask_member_off = None

    if bitmask_member_prop is not None:
        bitmask_member_off = mem.try_u32(
            bitmask_member_prop + UPROPERTY_OFFSET,
            None,
        )

    if bitmask_member_off is None:
        # Fallback: exact name under Core, preferring parent path BoolProperty.
        for o in _find_objects_named(
            objs,
            groups,
            "BitMask",
            package="Core",
            ...
            try:
                p = objs.path(o)
                if p.endswith("BoolProperty.BitMask"):
                    bitmask_member_prop = o
                    bitmask_member_off = mem.try_u32(
                        o + UPROPERTY_OFFSET,
                        None,
                    )
    ...
    if bitmask_member_off is None:
        print(
            "   !! Core.BoolProperty.BitMask не найден; "
            "bNetInitialRotation value не подтверждён"
        )
    else:
        print(
            "   UBoolProperty::BitMask native member offset = +0x%X"
            % bitmask_member_off
        )

        rotation_prop = _find_object_by_path(
            objs,
            groups,
        ...
        if rotation_prop is None:
            ...
            if prop_off is None or mask is None:
                print(
                ...
                if raw_value is None:
                    print(
                        "   !! не удалось прочитать CDO "
                        "bNetInitialRotation storage"
                    )
                else:
...
def main():
    ...
    if off is None:
        ...
        if not expects and not a.probe_package_net and not a.probe_packagemap and not a.probe_package_guids and not a.probe_playercontroller_open:
            print(
    ...
    if off is None:
        if expects:
            ...
            if off is None:
                return 1
        elif a.probe_package_net or a.probe_packagemap or a.probe_package_guids or a.probe_playercontroller_open:
